...
export default function InquiryDetailModal({
  ...
  // 서버 상세 응답에 secret이 없으므로 기본값 false (서버가 주면 초기화에 반영)
  const [secret, setSecret] = useState(false);
  const [submitting, setSubmitting] = useState(false);

  // ➜ AuthStore 구조에 맞게: token 유무만 사용 (권한은 서버가 검증)
  const token = useAuthStore((s) => s.token);
  const canEdit = !!token; // 로그인 상태에서만 수정/삭제 버튼 노출 (본인 여부는 서버가 403 등으로 판단)
  ...
  const load = useCallback(async () => {
    try {
      ...
      // setSecret(d.secret ?? false); // 서버가 secret을 내려주면 활성화
    } catch (e: any) {
  ...
  const onSubmitEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 서버가 본인 아닌 경우 403/에러 메시지로 거절할 것
      ...
      <div className="absolute inset-0 flex items-center justify-center p-4">
        <div className="w-full max-w-[720px] max-h-[80vh] bg-[#141420] border border-[#353535] rounded-xl shadow-xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#353535]">
            ...
            <button className="text-[#bbb] hover:text-white" onClick={onClose}>✕</button>
          </div>
              ...
              <>
                ...
                {detail.answerId > 0 && (
                  <div className="mt-6 p-3 rounded bg-[#1b1b27] border border-[#353535]">
                    <div className="text-xs text-[#7db7cd] font-semibold mb-1">
                      판매자 답변 · {detail.answerAuthorNickname}
                    </div>
                    <div className="text-sm text-[#ddd] whitespace-pre-wrap">
                      {detail.answerContent}
                    </div>
                    <div className="mt-1 text-[11px] text-[#888]">
                      {new Date(detail.answerCreatedAt).toLocaleString()}
                    </div>
                  </div>
                )}
                ...
                <div className="mt-6 flex gap-2 justify-end">
                  {canEdit && (
                    <>
                      ...
                      <button
                        className="px-4 py-2 text-sm border border-[#653e3e] text-[#ff9a9a] rounded hover:bg-[#2b1d1d]"
                        onClick={onDelete}
                        disabled={submitting}
                      >
                        삭제
              ...
              <form onSubmit={onSubmitEdit} className="flex flex-col gap-5">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#888] mt-1">
                    * 현재 상세 응답에 secret 필드가 없어 기본값으로 처리합니다.
                  </p>
                </div>
