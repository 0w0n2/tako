                        ...
                        rounded-full text-[14px] flex gap-1 items-center font-weight hover:bg-green-600"
								...
								<HoverCardContent className="">
										...
										<div className="flex flex-col gap-4">
											{history && history.length > 0 ? (
												history.map((item, index) => (
													<div
														key={`${item.timestamp}-${item.priceInEth}-${item.seller?.nickname ?? ""}-${item.buyer?.nickname ?? ""}`}
                                  ...
                                  ${index !== history.length - 1 ? "opacity-30 hover:opacity-100 transition-opacity duration-200" : ""}`}
													>
														{/* 연결선 */}
														{index !== history.length - 1 && <div className="absolute top-[100%] right-1/2 -translate-x-1/2 h-4 w-[2px] bg-[#a5a5a5]" />}
