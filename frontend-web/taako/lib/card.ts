...
// 카드 등급별 일자별 낙찰가 통계 (기본 7일)
export async function getCardGradeDailyStats(cardId: number | string, opts?: { days?: number; signal?: AbortSignal }) {
	const params: Record<string, any> = {};
	if (opts?.days) params.days = opts.days;
	const res = await api.get(`/v1/auctions/cards/${cardId}/grade-daily-stats`, { params, signal: opts?.signal });
	return res.data;
}
