...
/**
 * 배송 스키마 자동 생성 시 적용되는 @Table 메타데이터: - @UniqueConstraint uk_delivery_tracking_no
 * : tracking_number 고유 제약 생성 - @Index idx_delivery_sender : sender_address_id
 * 인덱스 생성 - @Index idx_delivery_recipient : recipient_address_id 인덱스 생성 - @Index
 * idx_delivery_status : status 인덱스 생성
 */
@Entity
@Table(name = "delivery", uniqueConstraints = {
        @UniqueConstraint(name = "uk_delivery_tracking_no", columnNames = "tracking_number") }, indexes = {
...
public class Delivery extends BaseEntity {
    ...
    /** 보내는 주소지 (선택) — 한쪽만 먼저 설정될 수 있으므로 DB 제약을 완화 */
    @ManyToOne(fetch = FetchType.LAZY, optional = true)
    @JoinColumn(name = "sender_address_id", nullable = true)
    private Address senderAddress;

    /** 받는 주소지 (선택) — 한쪽만 먼저 설정될 수 있으므로 DB 제약을 완화 */
    @ManyToOne(fetch = FetchType.LAZY, optional = true)
