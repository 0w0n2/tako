...
public class AuctionDetailRepositoryImpl implements AuctionDetailRepository {
    ...
    public AuctionInfo mapAuctionInfo(Auction a) {
        return AuctionInfo.builder()
                ...
                .end(a.isEnd())
                .buyNowFlag(a.isBuyNowFlag())
                .buyNowPrice(a.getBuyNowPrice())
    ...
    public List<String> findImageUrlsByAuctionId(Long auctionId) {
        return queryFactory
                ...
                .from(media)
                .where(
                        media.type.eq(MediaType.AUCTION_ITEM)
                                .and(media.ownerId.eq(auctionId))
                                .and(media.mediaKind.eq(MediaKind.IMAGE)))
                .orderBy(media.seqNo.asc(), media.id.asc())
                .fetch();
    }
    ...
    public List<DailyPriceLine> findWeeklyPriceLinesByCardId(Long cardId) {
        ...
        String sql = "SELECT DATE(ar.created_at) AS d, " +
                "       MIN(ab.bid_price) AS min_price, " +
                "       MAX(ab.bid_price) AS max_price, " +
                "       AVG(ab.bid_price) AS avg_price " +
    ...
    public List<BidHistoryItem> findBidHistory(Long auctionId, int limit) {
        List<Tuple> tuples = queryFactory
                .select(auctionBid.createdAt, auctionBid.bidPrice, member.nickname)
                .from(auctionBid)
                .join(auctionBid.member, member)
                .where(auctionBid.auction.id.eq(auctionId))
                .orderBy(auctionBid.createdAt.desc())
                .limit(limit)
                .fetch();

        return tuples.stream()
                .map(t -> BidHistoryItem.builder()
                        .createdAt(t.get(auctionBid.createdAt))
    ...
    public SellerInfo findSellerInfoByAuctionId(Long auctionId) {
        ...
        Tuple agg = queryFactory
                .select(auctionReview.id.count(), auctionReview.star.avg())
                .from(auctionReview)
        ...
        String profileImageUrl = queryFactory
                .select(media.url)
                .from(media)
                .where(media.type.eq(MediaType.MEMBER_PROFILE)
                        .and(media.ownerId.eq(sellerId))
                        .and(media.mediaKind.eq(MediaKind.IMAGE)))
                .orderBy(media.seqNo.asc(), media.id.asc())
                .fetchFirst();
